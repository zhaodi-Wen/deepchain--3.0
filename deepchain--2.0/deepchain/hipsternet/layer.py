...
def bn_forward(X, gamma, beta, cache, momentum=.9, train=True):
    ...
    if train:
        ...
        #print('gamma shape {},beta shape {} X_norm shape {}'.format(gamma.shape,beta.shape,X_norm.shape))
...
def  conv_bn_forward(X,gamma,beta,cache,momentum=.9,train=True):
    '''
    running_mean,running_var = cache
    N,C,H,W = X.shape
    ...
    #print('gamma shape {},beta shape {}'.format(gamma.shape,beta.shape))
    running_mean,running_var = cache
    N,C,H,W = X.shape
    X_flat = X.transpose(0,2,3,1).reshape(-1,C)
    out_flat,cache,running_mean,running_var = bn_forward(X_flat,gamma, beta, cache, momentum=.9, train=True)
    out = out_flat.reshape(X.shape).transpose(0,1,2,3)
...
def  conv_bn_backward(dout,cache):
    ...
    dx_flat,dgamma,dbeta = bn_backward(dout_flat,cache)
    dx = dx_flat.reshape(N,H,W,C).transpose(0,3,1,2)
    
    return dx,dgamma,dbeta
    

def conv_forward(X, W, b, stride=1, padding=2):
...
def maxpool_backward(dout, cache):
    def dmaxpool(dX_col, dout_col, pool_cache):
        ...
        #print('maxpool back shape',dX_col.shape)
        return dX_col
